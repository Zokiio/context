...
func PrepareSetup(ctx context.Context, request SetupRequest) (*SetupPlan, error) {
	...
	savedRoots := make([]any, 0, len(roots))
	...
	plan := &SetupPlan{target: target, before: before,
	...
	if plan.summary.Change != SetupUnchanged {
		plan.document, err = setupDocument(config, entry)
		if err != nil {
			return nil, fmt.Errorf("preserve setup configuration %s: %w", destination, err)
		}
	}
	plan.proposed, err = ParseConfig(destination, []byte(plan.document), profile)
	if err != nil {
		return nil, fmt.Errorf("validate proposed setup configuration: %w", err)
	}
	if plan.summary.Change != SetupUnchanged && len(config.Raw) != 0 && !sameSetupMetadata(config.Metadata, plan.proposed.Metadata) {
		return nil, fmt.Errorf("preserve setup configuration %s: YAML rewrite would change unrelated metadata", destination)
	}
	scope, err := resolve(ctx, plan.request, plan.proposed)
	if err != nil {
		return nil, fmt.Errorf("proposed setup binding conflicts with or cannot use effective declarations: %w", err)
	}
	if scope.Project == nil || !SamePath(scope.BindingDirectory, directory) || !SamePath(scope.Project.Records, records) || !sameSetupPaths(scope.Project.AllowSources, roots) {
		return nil, fmt.Errorf("proposed setup configuration %s does not select the requested directory, records, and allowed sources", destination)
	}
	return plan, nil
}
...
func setupEntry(config *Config, directory string, request SetupRequest) (map[string]any, *ProjectBinding, string, error) {
	...
	alias := ""
	if index >= 0 {
		entry = maps.Clone(entries[index].(map[string]any))
		entries[index] = entry
	...
	config.Metadata["projects"] = entries
...
func sameSetupPaths(left, right []string) bool {
	if len(left) != len(right) {
		return false
	}
	for _, path := range left {
		if !containsSetupPath(right, path) {
			return false
		}
	}
	return true
