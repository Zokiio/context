...
2. Capture the exact task-context JSON that this session reads. With the prepared project binding, use:
   ...
   The binding supplies the selected records and allowed source roots. For explicit direct access, replace `--project` with `--bundle` and repeat `--allow-source` for each authorized root. Keep the output bytes unchanged, read those bytes as the session's task context, and retain the exit status. An incomplete result can be retained, but cannot support a complete comparison.
