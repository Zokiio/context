...
- An explicitly empty required section declares none. A missing required section leaves the corresponding information unknown. In link-only sections, a nonempty prose-only declaration must be the literal word None; other prose without usable links is diagnosed rather than interpreted as an empty list. Goals may also contain authored prose.
