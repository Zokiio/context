...
The generated repository `.context/config.md` is retained. Its lock sidecar is ignored. The trial verifies both complete orientation with explicit larger budgets and the expected partial report at preserved default budgets.

The README now starts with cwd discovery. Separate how-to and reference documents cover setup, configuration, precedence, aliases, source authorization, workspace commands, failures, and migration to `--bundle`. The prior detailed reader reference is preserved in `docs/readers.md`. [Documentation evidence](evidence/06-documentation.json) retains document hashes, all 20 passing shell syntax checks, resolved local links, and the documentation implementer's CLI example checks. Independent review remains outstanding before final acceptance.
