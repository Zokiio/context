...
''')
        ...
        read('current-source-limit', args + ['--max-files', '1'], 1)
        limited, _ = read('cache-entry-limit', args + ['--max-cache-files', '1'], 1)
        assert not limited['recovery']['candidates']
        read('explicit-limit-overrides', args + ['--max-files', '1000', '--max-cache-files', '1000', '--max-cache-bytes', '16777216'])
    ...
    Path(output).write_text(json.dumps(dict(kind='controlled-fixtures', historicalTestedRevision=tested_revision, cases=rows), indent=2) + '\n')
    for row in rows:
...
if __name__ == '__main__':
    ...
    run(options.binary, options.output)
