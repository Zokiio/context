...
When discovery is not configured, select the record store directly. The ticket
path is relative to that store. A direct `--bundle` invocation also needs the
...
For this repository, the external documents selected by this task live under
the repository root. The following command was run against the current local
records while drafting this guide. It exited `0` with complete current context
and orientation, an absent recovery state, and no candidates.

```sh
/tmp/ctx resume \
  --bundle /Users/zoki/code/context/.scratch/records \
  --checkout /Users/zoki/code/context \
...
```sh
ctx resume --ticket feature/issues/07-verify-workflow.md \
...
Treat reported checks as historical observations. Re-run a check when the
current sources or the relevant environment have changed.
...
The cache is disposable working material, not the project record.
