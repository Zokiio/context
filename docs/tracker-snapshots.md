...
3. Choose and record the run order before starting. Give each session only the same ticket path. Time launch to the first verifiably correct continuation plan, and separately to its first correct action. Report startup time and a session that fails to continue rather than inventing a successful timing. Keep later tests and record writing outside these recovery intervals.
