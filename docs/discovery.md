...
Project aliases select their saved entries independently of cwd. Alias selection requires available records and allowed roots, but an unavailable checkout does not prevent selection. Unrelated missing registered targets do not invalidate the registry's structure.
...
The summary precedes every write. Identical setup preserves the original file and creates no lock. Updates preserve unrelated entries, unknown metadata, Markdown body bytes, and file permissions. Setup never creates records or silently repairs malformed configuration.
